...
function priorityStyles(p: TaskPriority): {
  ...
  switch (p) {
    case "high":
      return {
        label: "High",
        dotClass: "bg-rose-400 shadow-[0_0_10px_-2px_rgb(251_113_133)]",
        pillClass:
    ...
    case "medium":
      return {
        label: "Medium",
        dotClass: "bg-amber-400 shadow-[0_0_8px_-2px_rgb(251_191_36)]",
        pillClass:
...
function SpotlightCard({
  ...
  return (
    <article
      className={cn(
        "group relative flex flex-col overflow-hidden rounded-2xl border border-white/[0.08] bg-[color-mix(in_oklab,var(--card)_92%,transparent)] p-5 backdrop-blur-md transition-[transform,box-shadow,border-color] duration-300",
        "shadow-[0_0_0_1px_color-mix(in_oklab,var(--primary)_28%,transparent),0_28px_70px_-38px_color-mix(in_oklab,var(--primary)_55%,transparent)]",
        "hover:-translate-y-0.5 hover:border-primary/35 hover:shadow-[0_0_0_1px_color-mix(in_oklab,var(--primary)_42%,transparent),0_36px_90px_-36px_color-mix(in_oklab,var(--primary)_65%,transparent)]",
      )}
    >
      <div
        className="pointer-events-none absolute -right-20 -top-24 size-56 rounded-full opacity-[0.45] blur-3xl transition-opacity duration-500 group-hover:opacity-[0.65]"
        style={{
          background:
            "radial-gradient(circle, color-mix(in oklab, var(--primary) 42%, transparent) 0%, transparent 70%)",
        }}
        aria-hidden
      />
      <div className="relative flex items-start justify-between gap-3">
        <div className="flex items-center gap-2">
          <span
            className={cn(
              "flex size-11 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-primary/25 via-primary/10 to-transparent font-mono text-lg font-bold tabular-nums text-primary ring-1 ring-primary/30",
              rank === 1 &&
                "from-primary/35 ring-primary/45 shadow-[0_0_28px_-10px_var(--primary)]",
            )}
            aria-hidden
          >
...
function RunnerRow({ task, index }: { task: Task; index: number }) {
  ...
  return (
    <div
      className={cn(
        "flex items-start gap-3 rounded-xl border border-white/[0.05] bg-[color-mix(in_oklab,var(--card)_55%,transparent)] px-3 py-3 backdrop-blur-sm transition-colors hover:bg-[color-mix(in_oklab,var(--card)_72%,transparent)]",
      )}
    >
...
export function DashboardPrioritiesSection() {
  ...
  return (
    <section
      className="relative overflow-hidden rounded-[1.35rem] border border-white/[0.07] bg-[color-mix(in_oklab,var(--card)_75%,transparent)] shadow-[inset_0_1px_0_0_rgb(255_255_255/0.06)] backdrop-blur-xl"
    ...
    >
      ...
      >
        <div
          className="absolute -left-32 top-0 size-[420px] rounded-full blur-[100px]"
          style={{
            background:
        ...
        />
        <div
          className="absolute -right-40 bottom-0 size-[380px] rounded-full blur-[90px]"
          style={{
            background:
              "radial-gradient(circle, color-mix(in oklab, var(--chart-3) 28%, transparent), transparent 65%)",
          }}
        />
      </div>
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="space-y-3">
            <div className="flex flex-wrap items-center gap-2">
              <span className="bg-primary/15 text-primary inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[10px] font-bold tracking-[0.2em] uppercase ring-1 ring-primary/25">
          ...
          <Link
            ...
            className={cn(
              ...
              "border-primary/25 bg-primary/[0.06] text-primary hover:bg-primary/15 gap-1.5 rounded-xl",
            )}
          >
            Task board
      ...
      <div className="relative px-5 py-6 sm:px-8 sm:py-8">
        {ranked.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-3 py-14 text-center">
            <div className="bg-primary/12 text-primary flex size-14 items-center justify-center rounded-2xl ring-1 ring-primary/25">
            ...
            <Link
              href="/tasks"
              className={cn(buttonVariants({ variant: "default" }), "mt-2 gap-1.5 rounded-xl")}
            >
              Open tasks
              <ArrowRight className="size-4" aria-hidden />
            </Link>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              ...
              {spotlight.length < 3
                ? Array.from({ length: 3 - spotlight.length }).map((_, i) => (
                    <div
                      key={`placeholder-${i}`}
                      className="flex min-h-[200px] flex-col items-center justify-center rounded-2xl border border-dashed border-white/[0.08] bg-white/[0.02] px-4 text-center"
                    >
